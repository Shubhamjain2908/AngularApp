...
export class RecipeEditComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.params.subscribe(
      (params: Params) => {
        this.id = +params['id'];
        this.editMode = params['id'] != null;     // To check in which mode we are working ( edit or new)
        this.initForm();
      }
    );
  }
  ...
  onSubmit() {
    ...
    // );
    if (this.editMode) {
      this.recipeService.updateRecipe(this.id, this.recipeForm.value);  // we can also pass the new Recipe object as created above
    } else {
